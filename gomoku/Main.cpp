...
{
	int grid_size = 9;
	GameManager *gm = new GameManager(grid_size, grid_size);
	Player *turn;	
	turn = gm->getP1();
	int windowWidth = 800;
	...
	GraphicManager grm(&window, grid_size, windowWidth, windowHeight);
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			{	
				...
				{
					grm.menuClick(xClicked, yClicked, currentWindow, grid_size);
					grm.setSize(grid_size);
					grm.generateCells();	
					gm = new GameManager(grid_size, grid_size);
					turn = gm->getP1();
				}
